...
export class MatrixBgDirective implements AfterViewInit, OnDestroy {
  ...
  private initCanvas() {
    ...
    this.renderer.insertBefore(this.ele.nativeElement, this.canvas, this.ele.nativeElement.firstChild);
  }
  ...
  private draw() {
    ...
    this.context.fillRect(0, 0, this.ele.nativeElement.offsetWidth, this.ele.nativeElement.offsetHeight);

    this.context.fillStyle = this.colors[Math.floor(Math.random() * this.colors.length)];
    this.context.font = this.fontSize + 'px monospace';

    for (let i = 0; i < this.rainDrops.length; i++) {
      const text = this.katakana.charAt(Math.floor(Math.random() * this.katakana.length));
      this.context.fillText(text, i * this.fontSize, this.rainDrops[i] * this.fontSize);

      if (this.rainDrops[i] * this.fontSize > this.ele.nativeElement.offsetHeight && Math.random() > 0.975) {
        this.rainDrops[i] = 0;
      }
      this.rainDrops[i]++;
    }
